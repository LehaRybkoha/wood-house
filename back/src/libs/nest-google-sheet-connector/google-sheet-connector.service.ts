...
export class GoogleSheetConnectorService {
  ...
  async loadSheet(
    ...
    if (
      !this._loadedSpreadsheet ||
      this._loadedSpreadsheet.spreadsheetId !== spreadsheetId
    ) {
  ...
  async readRangeFromLoadedSheet(range: string): Promise<any[][]> {
    return this.readRangeFromLoadedSpreadSheet(
      this.getLoadedSheet().properties.title,
      range,
    );
  }
  ...
  private getRangeData(
    sheetData: sheets_v4.Schema$RowData[],
    range: string,
  ): any[][] {
  ...
  private async getSheetByLoadedSpreadSheetIndex(
    index: number,
  ): Promise<sheets_v4.Schema$Sheet> {
    return this._loadedSpreadsheet.sheets[index];
  }

  private async getSheetBySpreadSheetIndex(
    spreedsheatId: string,
    index: number,
  ): Promise<sheets_v4.Schema$Sheet> {
  ...
  private async getSheetMergeMetadata(sheet: sheets_v4.Schema$Sheet) {
  ...
  private async insertDimension(
    ...
    // Update spreadsheet.
    return this.getGoogleSheetConnect().spreadsheets.batchUpdate({
      spreadsheetId: spreadsheetId,
      requestBody: {
        requests: [request],
      },
  ...
  private async mergeCells(
    ...
    return await this.getGoogleSheetConnect().spreadsheets.batchUpdate({
      spreadsheetId: spreadsheetId,
      requestBody: {
        requests: [request],
      },
  ...
  async appendRow(
    ...
    return await this.getGoogleSheetConnect().spreadsheets.values.append(
      request,
    );
  }
  ...
  private async getColumnValues(
    ...
    const values =
      await this.getGoogleSheetConnect().spreadsheets.values.batchGet({
        spreadsheetId: spreedsheatId,
